...
What separates it from production is **not features — it is engineering
infrastructure**: no CI/CD, no tags/releases, no branch protections, no fastlane,
no signing pipeline, secrets hygiene debt, unconfirmed backend contracts for
every write-side flow (OTP bodies, leads, subscriptions, AI), and store readiness
...
1. **Secrets**: Google Maps keys existed in git history (removed by commit
   ...
   `google-services.json` is committed; no secret scanning anywhere.
...
fully green tree on every machine (including CI), with zero product risk.
